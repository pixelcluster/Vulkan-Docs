...
include::{generated}/api/structs/VkPhysicalDeviceFragmentShaderBarycentricPropertiesKHR.adoc[]
  ...
  * [[limits-triStripVertexOrderIndependentOfProvokingVertex]]
    pname:triStripVertexOrderIndependentOfProvokingVertex indicates that the
    implementation does not change its vertex numbering for triangle strip
    primitives when the <<vertexpostproc-flatshading,provoking vertex mode>>
    is ename:VK_PROVOKING_VERTEX_MODE_LAST_VERTEX_EXT, as shown in the
...
[open,refpage='VkPhysicalDeviceMemoryInitializationControlPropertiesFROG',desc='Properties of a physical device for memory initialization control',type='structs']
--
The sname:VkPhysicalDeviceMemoryInitializationControlPropertiesFROG structure
is defined as:
...
include::{generated}/api/structs/VkPhysicalDeviceMemoryInitializationControlPropertiesFROG.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:supportedMemoryTypes is a bitmask containing one bit set for every
    memory type that supports application-controlled memory initialization.
  * pname:lowThroughputFillMemoryTypes is a bitmask containing one bit set for
    every memory type where the implementation uses low-throughput operations
    to initialize device memory in the background.

Implementations should: set bits in pname:lowThroughputFillMemoryTypes only if
initializing memory explicitly, for example via flink:vkCmdFillBuffer, is
expected to yield significantly higher throughput compared to the
implementation's default initialization mechanisms.
